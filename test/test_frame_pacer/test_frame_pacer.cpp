...
static FramePacer settled(uint32_t& t)
{
    FramePacer p;
    t = 1000u;
    const FramePacer::Decision d = p.decide(t);   // the boot frame
...
void test_idle_holds_the_frame_until_the_cap(void) {
    ...
    TEST_ASSERT_EQUAL_INT((int)What::Nothing, (int)p.decide(t + 1u).what);
    TEST_ASSERT_EQUAL_INT((int)What::Nothing, (int)p.decide(t + REFRESH_MS - 1u).what);
    TEST_ASSERT_EQUAL_INT((int)What::Frame,   (int)p.decide(t + REFRESH_MS).what);
}
...
void test_a_change_draws_at_once(void) {
    ...
    TEST_ASSERT_EQUAL_INT((int)What::Nothing, (int)p.decide(t + 1u).what);
    p.changed();
    TEST_ASSERT_EQUAL_INT((int)What::Frame, (int)p.decide(t + 2u).what);
}
...
void test_the_panel_unrolls_dwells_and_hands_the_screen_back(void) {
    ...
    TEST_ASSERT_EQUAL_INT((int)What::Frame, (int)p.decide(t + DWELL_MS).what);
}
...
void test_an_animating_overlay_runs_at_the_animation_cadence(void) {
    uint32_t t = 0; FramePacer p = settled(t);
    p.overlay(Overlay::Panel, t);
    p.decide(t);                                        // the opening frame
    TEST_ASSERT_EQUAL_INT((int)What::Nothing,    (int)p.decide(t + ANIM_FRAME_MS - 1u).what);
    TEST_ASSERT_EQUAL_INT((int)What::FramePanel, (int)p.decide(t + ANIM_FRAME_MS).what);
}
...
void test_a_settled_overlay_idles_at_the_slower_cap(void) {
    ...
    p.decide(settled_at);
    TEST_ASSERT_EQUAL_INT((int)What::Nothing,    (int)p.decide(settled_at + ANIM_FRAME_MS).what);
    TEST_ASSERT_EQUAL_INT((int)What::FramePanel, (int)p.decide(settled_at + REFRESH_MS).what);
}
...
void test_reopening_the_panel_does_not_replay_the_unroll(void) {
    uint32_t t = 0; FramePacer p = settled(t);
    p.overlay(Overlay::Panel, t);
    TEST_ASSERT_EQUAL_UINT32(256u, p.decide(t + ANIM_MS).arg);

    const uint32_t again = t + 300u;
    p.overlay(Overlay::Panel, again);
    const FramePacer::Decision d = p.decide(again);
    ...
    TEST_ASSERT_EQUAL_INT((int)What::FramePanel, (int)p.decide(again + DWELL_MS - ANIM_MS - 1u).what);
    TEST_ASSERT_EQUAL_INT((int)What::Frame,      (int)p.decide(again + DWELL_MS - ANIM_MS).what);
}
...
void test_a_banner_is_paced_like_the_panel(void) {
    ...
    TEST_ASSERT_EQUAL_INT((int)What::FrameBanner, (int)p.decide(t).what);
    TEST_ASSERT_EQUAL_UINT32(256u, p.decide(t + ANIM_MS).arg);
    TEST_ASSERT_EQUAL_INT((int)What::Frame, (int)p.decide(t + DWELL_MS).what);
}
...
void test_the_save_confirmation_is_a_still_frame_held_for_the_dwell(void) {
    ...
    const FramePacer::Decision d = p.decide(t + 200u);
    ...
    p.decide(t + 300u);
    TEST_ASSERT_EQUAL_INT((int)What::Nothing,   (int)p.decide(t + 300u + ANIM_FRAME_MS).what);
    TEST_ASSERT_EQUAL_INT((int)What::FrameSave, (int)p.decide(t + 300u + REFRESH_MS).what);

    // It hands the screen back on the same dwell every other transient uses.
    TEST_ASSERT_EQUAL_INT((int)What::FrameSave, (int)p.decide(t + DWELL_MS - 1u).what);
    TEST_ASSERT_EQUAL_INT((int)What::Frame,     (int)p.decide(t + DWELL_MS).what);
}
...
void test_the_splash_animates_for_its_hold(void) {
    ...
    TEST_ASSERT_EQUAL_INT((int)What::Nothing, (int)p.decide(t).what);
    const FramePacer::Decision d = p.decide(t + ANIM_FRAME_MS);
    TEST_ASSERT_EQUAL_INT((int)What::SplashFrame, (int)d.what);
    TEST_ASSERT_EQUAL_UINT32(ANIM_FRAME_MS, d.arg);            // ms since it began

    TEST_ASSERT_EQUAL_INT((int)What::SplashFrame, (int)p.decide(t + SPLASH_MS - 1u).what);
    // Expired: the screen underneath is owed a frame.
    TEST_ASSERT_EQUAL_INT((int)What::Frame, (int)p.decide(t + SPLASH_MS).what);
}
...
void test_the_splash_suppresses_the_screen_underneath(void) {
    ...
    p.changed();
    p.changed();
    TEST_ASSERT_EQUAL_INT((int)What::SplashFrame, (int)p.decide(t + ANIM_FRAME_MS).what);
    TEST_ASSERT_EQUAL_INT((int)What::Frame,       (int)p.decide(t + SPLASH_MS).what);
}
...
void test_a_faulted_boot_runs_the_fault_then_the_splash(void) {
    ...
    TEST_ASSERT_EQUAL_INT((int)What::Nothing, (int)p.decide(t + ANIM_FRAME_MS).what);
    TEST_ASSERT_EQUAL_INT((int)What::Nothing, (int)p.decide(t + FAULT_HOLD_MS - 1u).what);
    TEST_ASSERT_EQUAL_INT((int)What::SplashRestart, (int)p.decide(t + FAULT_HOLD_MS).what);
    ...
    p.splash(splash_at);
    TEST_ASSERT_EQUAL_INT((int)What::SplashFrame, (int)p.decide(splash_at + ANIM_FRAME_MS).what);
    TEST_ASSERT_EQUAL_INT((int)What::Frame,       (int)p.decide(splash_at + SPLASH_MS).what);
}
...
void test_the_fault_hold_queues_the_splash_only_once(void) {
    ...
    p.fault_hold(t);
    TEST_ASSERT_EQUAL_INT((int)What::SplashRestart, (int)p.decide(t + FAULT_HOLD_MS).what);
    TEST_ASSERT_EQUAL_INT((int)What::Frame, (int)p.decide(t + FAULT_HOLD_MS + 1u).what);
}
...
void test_a_slide_captures_steps_and_settles(void) {
    ...
    const FramePacer::Decision first = p.decide(t + REFRESH_MS);
    ...
    const FramePacer::Decision step = p.decide(began + ANIM_FRAME_MS);
    ...
    TEST_ASSERT_EQUAL_INT((int)What::SlideSettle, (int)p.decide(began + FramePacer::SLIDE_MS).what);
    // Settling leaves the screen owed a frame of its own.
    TEST_ASSERT_EQUAL_INT((int)What::Frame, (int)p.decide(began + FramePacer::SLIDE_MS + 1u).what);
}
...
void test_a_slide_runs_for_the_duration_it_was_given(void) {
    ...
    const uint32_t began = t + REFRESH_MS;
    p.decide(began);                                    // capture, and the first step
    const FramePacer::Decision step = p.decide(began + ANIM_FRAME_MS);
    TEST_ASSERT_EQUAL_INT((int)What::SlideStep, (int)step.what);
    TEST_ASSERT_EQUAL_UINT32(ANIM_FRAME_MS, step.arg);

    // It ends on its own duration, where SLIDE_MS would still have it stepping.
    TEST_ASSERT_EQUAL_INT((int)What::SlideStep, (int)p.decide(began + fast - ANIM_FRAME_MS).what);
    TEST_ASSERT_EQUAL_INT((int)What::SlideSettle, (int)p.decide(began + fast).what);
    TEST_ASSERT_EQUAL_INT((int)What::Frame, (int)p.decide(began + fast + 1u).what);
}
...
void test_a_slide_without_a_duration_runs_for_the_default(void) {
    ...
    p.decide(again);
    TEST_ASSERT_EQUAL_INT((int)What::SlideStep, (int)p.decide(again + 40u).what);
    TEST_ASSERT_EQUAL_INT((int)What::SlideSettle,
                          (int)p.decide(again + FramePacer::SLIDE_MS).what);
}
...
void test_a_slide_steps_at_the_animation_cadence(void) {
    ...
    const uint32_t began = t + REFRESH_MS;
    p.decide(began);
    TEST_ASSERT_EQUAL_INT((int)What::Nothing,   (int)p.decide(began + ANIM_FRAME_MS - 1u).what);
    TEST_ASSERT_EQUAL_INT((int)What::SlideStep, (int)p.decide(began + ANIM_FRAME_MS).what);
}
...
void test_a_slide_closes_an_open_transient_first(void) {
    ...
    const uint32_t began = t + REFRESH_MS;
    p.decide(began);
    p.decide(began + FramePacer::SLIDE_MS);             // settle
    TEST_ASSERT_EQUAL_INT((int)What::Frame, (int)p.decide(began + FramePacer::SLIDE_MS + 1u).what);
}
...
void test_a_slide_is_refused_while_one_is_running(void) {
    ...
    p.decide(began + FramePacer::SLIDE_MS);
...
void test_a_slide_outranks_a_pending_change(void) {
    ...
    p.slide();
    p.changed();
    const FramePacer::Decision d = p.decide(t + REFRESH_MS);
